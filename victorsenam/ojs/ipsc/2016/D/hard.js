...
(function() {
    function p(a) {
        if (f)
            document.getElementById("log").value = document.getElementById("log").value + (a.pageX - a.target.offsetLeft) + " " + (a.pageY - a.target.offsetTop) + " " + (new Date).getTime() / 1E3 + "\n";
        return !1
    }
    ...
    next = function() {
        if (f) {
            ...
            d == commands.length ? (document.getElementById("log").value = document.getElementById("log").value + "done " +
                a + "\n", n.innerHTML = "Congratulations. Submit your action log.", f = !1) : (document.getElementById("log").value = document.getElementById("log").value + "next " + a + "\n", n.innerHTML = "<b>Level " + (d + 2) + ":</b> " + commands[d] + " Then click on the <b>Next</b> button.", d += 1, l());
        }
    };
