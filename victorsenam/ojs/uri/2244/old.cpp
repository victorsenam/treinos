...
bool cmp_t (int i, int j) {
    if (inp[i][cmp_k] != inp[j][cmp_k])
        return inp[i][cmp_k] < inp[j][cmp_k];
    if (inp[i][cmp_k+1] != inp[j][cmp_k+1])
        return inp[i][cmp_k+1] < inp[j][cmp_k+1];
    if (inp[i][(cmp_k^2)] != inp[j][(cmp_k^2)])
        return inp[i][(cmp_k^2)] < inp[j][(cmp_k^2)];
    if (inp[i][(cmp_k^2)+1] != inp[j][(cmp_k^2)+1])
        return inp[i][(cmp_k^2)+1] < inp[j][(cmp_k^2)+1];
    return 0;
}
...
int main () {
    ...
    for (int _i = 0; _i < n; _i++) {
        ...
        v[0].insert(inp[i][0], inp[i][1], i);              

        if (_i) {
            if (cmp_t(i, p[_i-1]) == cmp_t(p[_i-1], i)) {
                s.insert(i);
                s.insert(p[_i-1]);
            }
        }
    ...
    for (int j = 0; j < 2; j++) {
        for (int i = 1; i < v[j].ss; i++) {
            go(v[j].id[i-1], .5*(v[j].st[i] + v[j].st[i-1]));
        }
        go(v[j].id[v[j].ss-1], v[j].st[v[j].ss-1] + 1.);
    }
