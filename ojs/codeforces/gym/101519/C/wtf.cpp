	...
	while (ts--) {
		...
		while (k--) {
			...
			for (int i = 0; i < m; i++) {
				...
				usd[a] = usd[b] = turn;
				if (v[a].x < v[b].x)
					s_hi.pb(line(v[a],v[b]));
				else if (v[a].x > v[b].x) { // aqui quero maximo
					line l(v[a],v[b]);
					l.a *= -1; l.b *= -1;
					s_lo.pb(l);
				}
			}
			envelope<line> e_lo(0,2e9), e_hi(0,2e9);
			...
			for (int i = 0; i < n; i++) {
				...
				double lw = e_lo.q.size()?-e_lo.get(v[i].x)(v[i].x):-1./0.;
				double hg = e_hi.q.size()?e_hi.get(v[i].x)(v[i].x):1./0.;
