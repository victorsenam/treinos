...
struct lin { // line
	...
	double sign_dist (vec v) { return double(a*v.x + b*v.y - c)/sqrt(a*a + b*b); } // TODO
};
struct cir { // circle
	vec c; cood r;
...
int main () {
	...
	for (int i = 0; i < n; i++)
		scanf("%lf %lf %lf", &u[i].c.x, &u[i].c.y, &u[i].r);

	for (int i = 0; i < n; i++) {
		c += dfs(i);
		for (int j = i+1; j < n; j++) {
			try {
				pair<vec,vec> r = u[i].border_inter(u[j]);
				for (int a : {i,j,N-1}) { in[a].pb(r.first); in[a].pb(r.second); }
			} catch (int e) {
			}
		}
	}
	for (int i = 0; i < N; i++) {
		...
		int q = unique(in[i].begin(), in[i].end()) - in[i].begin();
		if (i < n)
			e += 1 + q;
		else if (i == N-1)
			v += n + q;
