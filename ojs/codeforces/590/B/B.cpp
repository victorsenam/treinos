...
template<typename cood> struct interval {
    ...
    template<typename ot_cood>
    interval<cood> (const interval<ot_cood> & ot) : a(ot.a), b(ot.b) {}
...
template<typename cood> struct vect {
    ...
    template<typename ot_cood>
    vect<cood> (const vect<ot_cood> & ot) : x(ot.x), y(ot.y) {}
...
template<typename cood> struct line {
    ...
    template<typename ot_cood>
    line<cood> (const line<ot_cood> & ot) : s(ot.s), t(ot.t) {}
...
template<typename cood> struct poly {
    ...
    template<typename ot_cood>
    poly<cood> (const poly<ot_cood> & ot) : v(ot.v) {}
...
template<typename cood> struct circ {
    ...
    template<typename ot_cood>
    circ<cood> (const circ<ot_cood> & ot) : c(ot.c), r(ot.r) {}
...
bool solve (double mi) {
    vect<double> curr = fr;

    if (mi >= t) {
        curr = curr + w[0]*t;
        curr = curr + vect<double>(w[1])*(mi-t);
    } else {
        curr = curr + vect<double>(w[0])*mi;
    }

    return curr.norm(to) <= mi*v;
}

int main () {
    scanf("%lld %lld %lld %lld", &fr.x, &fr.y, &to.x, &to.y);
    scanf("%lld %lld", &v, &t);
    for (int k = 0; k < 2; k++)
        scanf("%lld %lld", &w[k].x, &w[k].y);
