...
struct answ {
	...
	bool operator < (const answ & nw) const {
		return res < nw.res;
	}
};
...
int main () {
#ifdef ONLINE_JUDGE
	freopen("r", stdin, "illegal.in");
	freopen("w", stdout, "illegal.out");
#endif
	scanf("%d", &n);
	...
	set<vec> s;
	for (int i = 0; i < n; i++) {
		while (j < i && sq(v[j].x - v[i].x) > res.res) {
			s.erase(v[j]);
			j++;
		}
		...
		int qt = 12;
		while (it != s.end() && qt--)
			res = min(res, answ(v[i],*it++));
		qt = 12;
		while (jt != s.begin() && qt--)
			res = min(res, answ(v[i],*--jt));
