...
double antipodal (vec * p, int n, vec v) { // lg(n) | extreme segments relative to direction v TODO
...
int main () {
	while (scanf("%d", &n) != EOF) {
		...
		for (int i = 0; i < n; i++) {
			vec b = v[(i+1)%n], a = v[i];
			o = max(o,i+1);
			while (a.cross(b,v[o+1]) > a.cross(b,v[o])) o++;
			l = max(l,i+1);
			while (a.inner(b,v[l+1]) > a.inner(b,v[l])) l++;
			r = max(r,o);
			while (a.inner(b,v[r+1]) < a.inner(b,v[r])) r++;
			double da = a.cross(b,v[o])/a.nr(b);
			double db = (b-a)*(v[l]-v[r])/a.nr(b);
			//cout << da << " " << db << endl;
			res = min(res, da+da+db+db);
		}
		printf("%.20f\n", res);
	}
}
