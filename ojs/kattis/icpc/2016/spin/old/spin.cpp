...
struct evt {
	...
	bool operator < (const evt & ot) const {
		return vec(v).sd(vec(0,0),ot.v) > 0;
	}
};
...
int main () {
	...
	for (int i = 0; i < bs; i++) {
		...
		if (b[i].in_conv_poly(a, as, p, lo, hi))
			continue;

		insere(evt({ b[i] - a[lo], i }));
		insere(evt({ a[hi] - b[i], i }));
	}
	...
	for (int i = 0; i < ev.size(); ) {
		evt cur = ev[i];
		while (i < ev.size() && !(cur < ev[i])) {
			process(ev[i].i);
			i++;
		}

		res = min<int>(res, n - s.size());
	}
	printf("%d\n", res);
}
