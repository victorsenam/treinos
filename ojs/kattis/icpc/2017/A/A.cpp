...
void upd (bool & ok, bool & nl, vec & l, bool & nr, vec & r, vec a, vec b, vec x) {
    bool lala = 0;
    if (x.dr(a,b) == 1 && x.dr(b,a) == 1) {
        ok = 0;
        assert(!lala);
        lala = 1;
    }
...
int main () {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i+1; j < n; j++) {
            if (v[j].sd(v[i], v[i+1]) == 1) continue;
            if (v[j].sd(v[i+n-1], v[i]) == 1) continue;
