...
#define pb push_back
 ...
 typedef double cood; cood eps = 1e-9; cood inf = 1./0.; // risky: XXX, untested: TODO
...
double union_area (cir * v, int n) {
	struct I { vec v; int i; } c[2*(n+4)];
	srand(time(NULL)); double res = 0; vector<bool> usd(n);
	double lim = 1./0.; for (int i = 0; i < n; i++) lim = min(lim, v[i].c.y - v[i].r - 1);
	for (int i = 0, ss = 0; i < n; i++, ss = 0) {
		c[ss++] = {v[i].c + vec(1,0)*v[i].r, i}; c[ss++] = {v[i].c - vec(1,0)*v[i].r,i};
		c[ss++] = {v[i].c + vec(0,1)*v[i].r, i}; c[ss++] = {v[i].c - vec(0,1)*v[i].r,i}; // avoids angles = pi
		for (int j = 0; j < n; j++) if (!(v[i].c == v[j].c) && v[i].has_border_inter(v[j])) { // intersections with others (treats equal case)
			pair<vec,vec> r = v[i].border_inter(v[j]);
			c[ss++] = { r.first, j}; c[ss++] = { r.second, j};
		}
		double ang = rand(); // rotation avoids corner on cnt initialization
		sort(c, c+ss, [v,i,ang] (I a, I b) { return vec(0,0).compare((a.v-v[i].c).rotate(ang),(b.v-v[i].c).rotate(ang)); });
		vec fp = v[i].c + (~(c[0].v+c[ss-1].v-v[i].c-v[i].c))*v[i].r;
		int cnt = 0; for (int j = 0; j < n; j++) { cnt += (usd[j] = v[j].contains(fp)); }
		for (int j = 0; j < ss; j++) {
			if (c[j].i != i) { cnt -= usd[c[j].i]; usd[c[j].i] = !usd[c[j].i]; cnt += usd[c[j].i]; }
			vec a = c[j].v, b = c[(j+1)%ss].v;
			double cir = abs(v[i].arc_area(a,b) - v[i].c.cross(a,b)/2), tra = abs((b.x-a.x)*(a.y+b.y-2*lim)/2);
			double loc = (a.x<b.x)?cir-tra:tra+cir; res += (cnt==1)?loc:0;
		}
	}
	return res;
}
...
int main () {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < i; j++) if (v[i].c == v[j].c && abs(v[i].r - v[j].r) <= eps) { i--; n--; break; }
	}
	printf("%.5f\n", union_area(&v[0], n));
}
