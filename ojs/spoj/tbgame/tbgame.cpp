...
bool solve (vector<vect<double> > & v, int p) {
    int n = v.size();

    for (int x = 0; x < 4; x++) {
        int mx[2] = {0, 0};
        for (int i = 2; i < n; i++) {
            bool b = (v[p].clockwise(v[!p], v[i]) > 0);

            if (!mx[b] || v[p].clockwise(v[mx[b]], v[i]) == cond(b, x&1, x&2))
                mx[b] = i;
        }

        if (!mx[0] || !mx[1]) 
            return 1;
        if (!line<double>(v[0], v[1]).intersects(line<double>(v[mx[0]], v[mx[1]]), eps))
            return 1;
    }
    return 0;
}
