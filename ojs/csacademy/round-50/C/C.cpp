...
int main () {
	...
	while (ts--) {
		...
		do {
			ok = 1;
			//cout << v[p[0]].sq(v[p[3]]) << endl;
			for (int i = 0; i < 3; i++) {
				//cout << v[p[i]].sq(v[p[i+1]]) << endl;
				ok &= (v[p[i]].sq(v[p[i+1]]) == v[p[0]].sq(v[p[3]]));
			}
			//cout << v[p[0]].sq(v[p[2]]) << endl;
			//cout << v[p[1]].sq(v[p[3]]) << endl;
			ok &= (v[p[0]].sq(v[p[2]]) == v[p[1]].sq(v[p[3]]));
			ok &= (v[p[0]].sq(v[p[2]]) == 2ll*v[p[0]].sq(v[p[1]]));
		} while (!ok && next_permutation(p, p+4));
