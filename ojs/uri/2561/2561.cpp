...
typedef ll cood; cood eps = 0; cood inf = 1e14;
...
bool cov (vec a, vec b) {
	return (a.x <= b.x && a.y <= a.y);
}

bool covers (vec a, vec b) {
	return a.x <= b.x && a.y <= b.y;
}

int main () {
	vec a, b, c, d;
	while (scanf("%lld %lld %lld %lld", &a.x, &a.y, &b.x, &b.y) != EOF) {
		scanf("%lld %lld %lld %lld", &c.x, &c.y, &d.x, &d.y);

		vec l(max(a.x,c.x),max(a.y,c.y)), r(min(b.x,d.x),min(b.y,d.y));
		ll a = (r.x-l.x)*(r.y-l.y);
