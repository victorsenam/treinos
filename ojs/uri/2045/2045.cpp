...
int in_polygon (vector<vec> & p, vec v) { // p should be simple, borders included
	int in = -1;
	for (int i = 0; i < (int) p.size(); i++) {
		vec a = p[i], b = p[i?i-1:p.size()-1];
		if (v.in_seg(a,b)) return 0;
		if (a.x > b.x) swap(a,b);
		bool sw = ((b.x > v.x + eps) && (a.x <= v.x + eps) && v.ccw(a,b) < 0);
		if (sw) {
			in = -in;
		}
	}
...
int main () {
	for (int tt = 1; tt; tt++) {
		...
		for (int j = 0; j < m; j++) {
			...
			printf("soldado %d ", j+1);
			if (in_polygon(v, a) != -bd)
				printf("defender\n");
			else
				printf("espanhol\n");
		}
