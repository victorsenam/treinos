...
struct cir { // circle
	...
	bool operator < (const cir & o) const {
		if (abs(c.x - o.c.x) > eps) return c.x < o.c.x;
		if (abs(c.y - o.c.y) > eps) return c.y < o.c.y;
		if (abs(r - o.r) > eps) return r < o.r;
		return 0;
	}
	...
	while (scanf("%d", &n) != EOF && n) {
		...
		if (n <= 2) printf("%d\n", n);
			...
			for (int i = 0; i < n; i++)
				for (int j = i+1; j < n; j++)
					for (int k = j+1; k < n; k++) {
						//cout << i << " " << j << " " << k << ":" << v[i].cross(v[j],v[k]) << endl;
						if (v[i].ccw(v[j],v[k])) {
							ll x = (++mp[cir(v[i],v[j],v[k])]);
							mx = max(mx, x);
						}
					}
