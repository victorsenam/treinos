...
struct square {
	...
	double dist2 (square & o) {
		double mn = inf;
		vec s[4] = {v + vec(l/2,l/2), v + vec(l/2,-l/2), v + vec(-l/2,-l/2), v + vec(-l/2,l/2)};
		vec t[4] = {o.v + vec(o.l/2,o.l/2), o.v + vec(o.l/2,-o.l/2), o.v + vec(-o.l/2,-o.l/2), o.v + vec(-o.l/2,o.l/2)};

		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++) {
				mn = min(s[i].dist2_seg(t[j], t[(j+1)%4]), mn);
				mn = min(t[i].dist2_seg(s[j], s[(j+1)%4]), mn);
			}
		return mn;
	}
};
...
int main () {
	...
	square ls;
	scanf("%lf %lf %lf", &ls.v.x, &ls.v.y, &ls.l);
	for (int i = 1; i < n; i++) {
		square cur;
		scanf("%lf %lf %lf", &cur.v.x, &cur.v.y, &cur.l);

		if (!(cur.dist2(ls) <= aah*aah + eps)) {
			printf("OUCH\n");
			return 0;
		}
		ls = cur;
