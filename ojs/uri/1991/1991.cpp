...
struct cir { // circle
	...
	inline vec at_x (double x) {
		return vec(x, c.y + (up - !up)*sqrt(r*r - sq(x-c.x)));
	}
...
double go (double x, cir a, cir b) {
	return a.at_x(x).nr(b.at_x(x));
}
...
int main () {
	while (scanf("%lf", &a)) {
		...
		double mn = 1./0.;
		for (ll r : ev) {
			while (r > v[0][i].c.x + v[0][i].r + eps) {
				assert(i <= n[0] + 7);
				i++;
			}
			while (r > v[1][j].c.x + v[1][j].r + eps) {
				assert(j <= n[1] + 7);
				j++;
			}
			...
			while (qt--) {
				...
				if (go(q1,v[0][i],v[1][j]) < go(q2,v[0][i],v[1][j])) hi = q2;
				else lo = q1;
			}
			mn = min({ go(lo,v[0][i],v[1][j]), go(l,v[0][i],v[1][j]), go(r,v[0][i],v[1][j]), mn });
			l = r;
		}
