...
int main () {
	while (scanf("%d", &n) != EOF && n) {
		...
		double res = w*w;
		for (int i = 0; i < n; i++) {
			scanf("%lld %lld %lld", &v[i][0].y, &v[i][1].x, &v[i][1].y);
			if (i&1) {
				v[i][0].x = w;
				res = min(res, double(v[i][1].x*v[i][1].x));
			} else {
				v[i][0].x = 0;
				res = min(res, double(sq(w-v[i][1].x)));
			}

			if (i) res = min(res, dist2_seg(v[i][0],v[i][1],v[i-1][0],v[i-1][1]));
		}
