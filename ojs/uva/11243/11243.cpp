...
#define pb push_back
 ...
 typedef double cood; cood eps = 1e-8; cood inf = 1e15;; // risky: XXX, untested: TODO
...
double get (vec d) {
	pii a = antipodal(v, n, d);
	pii b = antipodal(v, n, d.rot90());
	return max(d*(v[a.first]-v[a.second]),d.rot90()*(v[b.first]-v[b.second]))/d.nr();
}
...
int main () {
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) { d.pb(v[(i+1)%n]-v[i]); d.pb(vec(0,0) - d.back()); }
		auto md = partition(d.begin(), d.end(), [] (vec v) { return v.x < -eps || (abs(v.x) <= eps && v.y > eps); });
		sort(d.begin(), md, [] (vec a, vec b) { return b.ccw(vec(0,0),a) > 0; });
		sort(md, d.end(), [] (vec a, vec b) { return b.ccw(vec(0,0),a) > 0; });
		//for (vec x : d) cout << x << endl;

		double res = 1./0.;
		for (int i = 0; i < d.size(); i++) {
			vec a = d[i], b = d[(i+1)%d.size()];
			
			double lo = 0, hi = 1;
			int ts = 50;
			res = min({ res, get(a), get(b) });
			while (ts--) {
				double p1 = (2*lo + hi)/3;
				double p2 = (lo + 2*hi)/3;
				if (get(a*p1 + b*(1.-p1)) < get(a*p2 + b*(1.-p2))) hi = p2;
				else lo = p1;
			}
			res = min(res, get(a*lo + b*(1-lo)));
		}
