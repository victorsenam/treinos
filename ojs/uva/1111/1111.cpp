...
inline ll dist (int i, int j) {
	if (j <= i) return 0;
	if (j >= i+n) return 0;
	return v[i%n].sq(v[j%n]);
}

ll go (int l, int r, int a, int b) {
	if (l > r) return 0;
	int p = (l+r)/2;

	int o = a;
	for (int k = a; k <= b; k++)
		if (dist(p,k) > dist(p,o)) o = k;

	return max({ dist(p,o), go(l,p-1,a,o), go(p+1,r,o,b) });
}
...
int main () {
	...
	while (scanf("%d", &n) != EOF && n) {
		...
		n = convex_hull(&v[0],n,0);

		printf("Case %d: %.2f\n", ++tt, sqrt(go(0,n-1,0,n+n-1)));
	}
}
