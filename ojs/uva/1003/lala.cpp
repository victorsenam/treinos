...
struct cir { // circle
	...
	inline vec cen_tan (cir o) { return (c*o.r + o.c*r)/(r + o.r); }
...
int main () {
	...
	int ts = 1;
	while (cin >> x[0] >> y[0] >> x[1] >> y[1] >> n) {
		for (int i = 0; i < n; i++) {
			cin >> c[i].c.x >> c[i].c.y >> c[i].r >> c[i].h;
			c[i].r /= 2;
		}
		...
		do {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < n; j++) if (i != j)
					pts.pb(c[i].cen_tan(c[j]));
				for (pair<vec,pair<vec,vec> > p : {
					{vec(c[i].r,0), vec(x[1]-c[i].r,y[0]),vec(x[1]-c[i].r,y[1])},
					{vec(0,c[i].r), vec(x[0],y[1]-c[i].r),vec(x[1],y[1]-c[i].r)},
					{vec(-c[i].r,0), vec(x[0]+c[i].r,y[0]),vec(x[0]+c[i].r,y[1])},
					{vec(0,-c[i].r), vec(x[0],y[0]+c[i].r),vec(x[1],y[1]+c[i].r)}
				}) {
					if (a.sq(b) <= eps && !border(a)) {
						pair<vec,vec> r = cir(c[i].c,c[i].h-c[i].r).border_inter_lin(p.second.first,p.second.second);
						pts.pb(r.first + p.first);
						pts.pb(r.second + p.first);
					}
				}
			}
