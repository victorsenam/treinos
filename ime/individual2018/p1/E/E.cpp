...
typedef ll cood; cood eps = 0; cood inf = 1./0.; // risky: XXX, untested: TODO
...
struct vec {
	...
	bool isin (vec a, vec b, vec c) {
		if (ccw(a,b)*ccw(b,c)*ccw(c,a) == 0) return 1;
		if (ccw(a,b) == ccw(b,c) && ccw(a,b) == ccw(c,a)) return 1;
		return 0;
	}
};
struct lin { // line
...
int go (int i, int j) {
	if (b[i].x < b[j].x) return cn[i][j][0];
	else return -cn[i][j][2];
}
	...
	while (scanf("%d %d %d", &n[0], &n[1], &n[2]) != EOF) {
		...
		for (int i = 0; i < n[2]; i++) {
			for (int j = i+1; j < n[2]; j++) {
				cn[i][j][0] = cn[i][j][2] = 0;
				ll mn = min(b[i].x,b[j].x), mx = max(b[i].x,b[j].x);
				//cout << i << " " << j << endl;

				for (int k = 0; k < m; k++) {
					if (v[k].x >= mn && v[k].x < mx) {
						//cout << k << " to the " << v[k].ccw(b[i],b[j])+1 << endl;
						cn[i][j][v[k].ccw(b[i],b[j])+1] += v[k].c?1:-1;
					}
				}

				//cout << "GIVES " << cn[i][j][0] << " " << cn[i][j][2] << endl;
		...
		for (int i = 0; i < n[2]; i++) {
			for (int j = i+1; j < n[2]; j++) {
				for (int k = j+1; k < n[2]; k++) {
					int sm = go(i,j) + go(j,k) + go(k,i);
					if (b[k].ccw(b[i],b[j]) == -1) sm = -sm;
					if (sm > 0) ra++;
					else if (sm < 0) rb++;
				}
			}
