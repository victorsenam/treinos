...
int main()
{
    ...
    for(auto it=h.begin();it!=h.end();)
    {
        ...
        r.insert(mp(mp(start, curr), 1));
        r.insert(mp(mp(reach, curr),0));
    ...
    for(auto it=v.begin();it!=v.end();)
    {
        num curr  = it->first,
            start = it->second.first,
            reach = it->second.second;
        while(!r.empty() && r.begin()->first.first <= curr)
        {
            int x = lower_bound(seen, seen+ss, r.begin()->first.second) - seen;
            change(x, r.begin()->second);
            r.erase(r.begin());
        }
        ...
        ans += (reach - start + num(1)) - get(lo, hi+1);
    }
    cout << ans << endl;
}
