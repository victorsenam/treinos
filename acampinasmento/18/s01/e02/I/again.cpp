...
void monotone_chain (vec * v, int n, int border_in, vector<vec> & r) { // nlg | border_in (should border points stay?)
	n = unique(v, v+n) - v;
...
pii antipodal (vec * p, int n, vec v) { // lg(n) | extreme points relative to direction v
...
vector<vector<vec> > v[2]; // sorted points, convex polygon

int main () {
	scanf("%d ", &n);
	v[0].pb(vector<vec>(n));
	v[1].pb(vector<vec>());
	for (int i = 0; i < n; i++) scanf("%lld %lld", &v[0][0][i].x, &v[0][0][i].y);
	sort(v[0][0].begin(), v[0][0].end());
	monotone_chain(&v[0][0][0], v[0][0].size(), 0, v[1][0]);

	scanf("%d", &m);
	for (int i = 0; i < m; i++) {
		scanf(" %s %lld %lld", qry, &cur.x, &cur.y);
		if (qry[0] == 'a') {
			vector<vec> nw({cur});
			while (v[0].size() && v[0].back().size() <= nw.size()) {
				vector<vec> tmp = nw; nw.resize(nw.size() + v[0].back().size());
				merge(tmp.begin(), tmp.end(), v[0].back().begin(), v[0].back().end(), nw.begin());
				v[0].pop_back();
				v[1].pop_back();
			}

			v[0].push_back(nw);
			v[1].push_back(vector<vec>());
			monotone_chain(&v[0].back()[0], v[0].back().size(), 0, v[1].back());
		} else {
			ll res = LLONG_MIN;
			for (vector<vec> & p : v[1]) {
				pii idx = antipodal(&p[0], p.size(), cur);
				res = max(res, cur*p[idx.first]);
			}
			printf("%lld\n", res);
		}
	}
