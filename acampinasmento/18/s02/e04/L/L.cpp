...
vector<pii> go(int u) {
	...
	for(pii ev : evs) {
		...
		ll time = nx_time - cur_time;
		while(cur_stock + (time - 1) * cur_supply < time * cur_speed) {
			ll mn = (cur_stock - cur_supply + (cur_speed - cur_supply - 1)) / (cur_speed - cur_supply);
			//assert(mn >= 0 && cur_time + mn < nx_time);
			ll old_speed = cur_speed;
			cur_speed = cur_stock + mn * cur_supply - (mn + 1) * cur_speed;
			cur_stock = cur_speed;
			cur_time += mn;
